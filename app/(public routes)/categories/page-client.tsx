...
export default function CategoriesPage({ initialPage, limit }: Props) {
	const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useInfiniteQuery({
		queryKey: ["categories"],
		queryFn: ({ pageParam = initialPage }) => getCategories(pageParam as number, limit),
	...
	return (
		<section className={css.pageContainer}>
			...
			{isInitialLoading && <p className={css.loadingText}>Завантаження початкових даних...</p>}

			{allCategories.length > 0 && <CategoriesList categories={allCategories} />}

			{hasNextPage && (
				<button type="button" onClick={handleLoadMore} disabled={isFetchingNextPage} className={css.loadMoreButton}>
