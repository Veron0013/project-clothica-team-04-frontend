...
export default function AuthComponent({ login = false }: AuthComponentProps) {
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      if (msg.includes('Необхідно ввести номер телефону та пароль')) {
        setFieldError('phone', 'Вкажіть телефон');
        setFieldError('password', 'Вкажіть пароль');
      } else if (msg.includes('Некоректний номер телефону')) {
        setFieldError('phone', msg);
      } else if (msg.includes('Цей номер телефону вже використовується')) {
        setFieldError('phone', msg);
      } else if (msg.includes('Невірний номер телефону або пароль')) {
        setFieldError('phone', msg);
