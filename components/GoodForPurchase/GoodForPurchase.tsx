...
export default function GoodForPurchase({ good }: GoodForPurchaseProps) {
  ...
  const animateMainImageToCart = () => {
    const cartIconEl = document.querySelector(
      '[data-cart-button="header-cart"]'
    ) as HTMLElement | null;

    if (!cartIconEl) {
      console.log('❌ Не знайшли кнопку кошика в Header (GoodForPurchase)');
      return;
    }
    if (!mainImageRef.current) {
      console.log('❌ mainImageRef.current нема');
      return;
    }
    if (!mainImage) {
      console.log('❌ mainImage порожній');
      return;
    }
