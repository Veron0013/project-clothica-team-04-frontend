...
export default function ReviewModal({ open, onClose, productId, category, onSubmitted }: ReviewModalProps) {
	...
	const isDisabled = submitting || !name.trim() || !text.trim() || rating < 1 || rating > 5 || !productId
	...
	return (
		<Modal open={open} onClose={onClose} ariaLabelledby="review-title">
			<div className={css.container}>
				...
				<button type="button" className={css.closeBtn} aria-label="Закрити модальне вікно" onClick={onClose}>
				...
				<form className={css.form} onSubmit={handleSubmit} noValidate>
					...
					<div className={css.ratingRow} aria-label="Оцінка">
						{[1, 2, 3, 4, 5].map((n) => (
							<StarBtn key={n} value={n} active={rating >= n} onClick={setRating} />
						))}
					</div>
