...
export const getReviews = async () => {
	const response = await nextServer.get("/feedbacks", { params: { page: 1, limit: 6 } })
...
export const createFeedbackClient = async (feedback: FeedbackPayload) => {
	try {
		...
		if (axios.isAxiosError(err)) {
			const msg = (err.response?.data as AxiosError)?.message ?? err.message ?? "Creating feedback failed"
			throw new Error(msg)
		}
